			...
			for (XNode child : parent.getChildren()) {
				if ("package".equals(child.getName())) {
					...
					if (resource != null && url == null) {
						...
						XMLGenMapperBuilder mapperParser = new XMLGenMapperBuilder(inputStream, configuration, resource,
								configuration.getSqlFragments());
						mapperParser.parse();
					} else if (resource == null && url != null) {
						ErrorContext.instance().resource(url);
						InputStream inputStream = Resources.getUrlAsStream(url);
						XMLGenMapperBuilder mapperParser = new XMLGenMapperBuilder(inputStream, configuration, url,
								configuration.getSqlFragments());
						mapperParser.parse();
					} else {
						throw new BuilderException(
